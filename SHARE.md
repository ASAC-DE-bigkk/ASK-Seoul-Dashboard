...
### 12.1 라벨 표준 — 플랫폼 역할 기준

**이 저장소의 성격을 먼저 못 박는다.** dashboard 는 메달리온 파이프라인(수집·적재·변환)이
**아니다.** 상류(dags/dbt)가 R2/Iceberg 에 **쌓아 둔 데이터를 소비**해 ① 데이터 서비스 제공
② 공개 API 서빙 ③ 분석·시각화를 하는 **플랫폼**이다. 따라서 라벨 어휘도 "적재 단계"가 아니라
**서비스·API·분석 행동**을 가리킨다(상류 dags/dbt 의 `area: ingestion` 류와 의도적으로 다르다).

라벨은 **운영 자산(실제 GitHub 라벨)인 동시에 문서**다 — 이 표가 정본이며, 새 라벨이 필요하면
이 표에 추가하고 **같은 PR 에서 `gh label create`/`edit` 으로 실제 라벨도 만든다**(코드-문서 동기).

**`type:` (작업 종류 — 이슈·PR 당 1개)** · 색상은 시맨틱

| 라벨 | 색 | 용도(이슈 접두) |
|---|---|---|
| `type: feature` | `#a2eeef` | 기능·서비스 개발 (`[Feat]`) |
| `type: bug` | `#d73a4a` | 버그 수정 (`[Bug]`) |
| `type: perf` | `#0e8a16` | 성능·응답성·캐시 — 서빙 플랫폼 관심사 |
| `type: refactor` | `#fbca04` | 리팩터·구조 개선(동작 불변) |
| `type: security` | `#b60205` | 보안·접근제어·취약점 대응 |
| `type: chore` | `#fef2c0` | 설정·잡무·정리·의존성 (`[Chore]`) |
| `documentation` | `#0075ca` | 문서 추가·개선 (`[Docs]`, GitHub 기본 라벨 유지) |

**`area:` (플랫폼 컴포넌트 — 1개 이상)** · 색상 균일 `#5319e7`(prefix 그룹핑)

| 라벨 | 플랫폼 행동·범위 |
|---|---|
| `area: catalog` | **데이터 서비스** — 카탈로그·소스 탐색, 축적 데이터 발견·메타 서빙 |
| `area: charts` | **분석·시각화** — Charts Studio(온톨로지·라이브 쿼리·필터·다중 백엔드) |
| `area: api` | **API 서빙** 공통 — 엔드포인트·응답 계약(RFC7807)·버전·레이트리밋 |
| `area: auth` | **계정 서비스** — 인증·권한·이용권·운영 콘솔(`app/auth/`) |
| `area: frontend` | **화면·UX** — 정적 자산(`app/static/`) |
| `area: data-sync` | **데이터 소비** — 서빙 스냅샷·소스 연결(축적 gold·외부 RDB 를 소비, **적재 아님**) |
| `area: platform` | **플랫폼 운영** — 배포·Compose·관측·CI |
...
### 12.2 이슈·PR 작성 포맷 — 서비스/API/분석 플랫폼 기준

파이프라인의 "적재 단계·데이터 레이어" 서술이 아니라, **소비자에게 어떤 서비스·API·분석
능력이 생기거나 바뀌는가**를 중심으로 쓴다. 아래 골격을 따른다(해당 없는 절은 생략 가능).

**이슈 골격**

```markdown
## 무엇을 · 사용자 가치
(어떤 데이터 서비스/API/분석 능력이 생기거나 개선되는가 — 소비자 관점 1~3줄)

## 대상 표면 (화면 · 엔드포인트 · 분석)
(예: /charts · POST /api/v1/charts/query · 필터 편집 UX)

## API · 계약 영향
(응답 계약/온톨로지/저장물 변화 여부, 하위호환 여부. 없으면 "없음")
...
## 검증
(엔드포인트·pytest·selftest·화면 — 무엇으로 증명하는가)

## 관련
(규약·문서·상류 계약 링크)
```

**PR 골격**: `Closes #N` → `## 요약`(사용자 가치) → `## 변경`(표면별) →
`## API·하위호환`(계약/캐시/저장물 영향) → `## 검증` → `## 관련`.

원칙: **소비자 가치 우선**(무엇을 할 수 있게 되는가) · **표면 명시**(어느 화면·엔드포인트) ·
**계약·하위호환을 항상 한 절로**(라이브 서빙이라 응답 계약·캐시·저장물 호환이 핵심) ·
검증은 "적재 성공"이 아니라 **"서비스가 그렇게 동작함"**을 보인다.
