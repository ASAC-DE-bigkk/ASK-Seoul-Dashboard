...
로컬 자동 인증 최초 반영은 `feat/13-local-analysis-auto-auth`/PR #14로 추적한다. 아직 `dev`에
merge되지 않은 시점에 다른 checkout을 준비해야 한다면 먼저 Dashboard worktree가 깨끗한지 확인하고,
사용자 요청 범위 안에서만 해당 branch를 checkout한다. 상위 submodule pointer를 임의로 commit하지 않는다.
