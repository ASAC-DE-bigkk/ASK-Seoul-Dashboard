...
로컬 자동 인증의 최초 반영 작업은 `feat/13-local-analysis-auto-auth` 브랜치와 PR #14에서 관리한다.
PR이 `dev`에 merge되기 전 다른 PC에서 시험할 때만, Dashboard 작업 트리가 깨끗한지 확인한 뒤 다음을
실행한다. 이미 수정 중인 파일이 있으면 branch를 바꾸지 말고 먼저 작업 소유자와 정리한다.

```bash
git -C dashboard fetch origin
git -C dashboard switch feat/13-local-analysis-auto-auth
```

PR merge 후에는 상위 저장소가 가리키는 Dashboard submodule commit 또는 Dashboard `dev`를 사용하며,
위 feature branch 전환은 필요하지 않다.
