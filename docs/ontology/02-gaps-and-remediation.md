...
| **재고 가산성 집행** | 재고성 측정값을 **여러 시점에 걸쳐** 합산하면 이중계산 → 거부. 반대로 시간축을 GROUP BY 에 두면 시각별 합이라 옳으므로 허용한다(방향을 뒤집으면 정상 시계열을 막고 진짜 이중계산을 통과시킨다 — 적대적 리뷰가 잡아낸 실제 결함). ratio·HAVING 도 같은 검사, `preferred_agg` 는 접을 시간축이 있을 때만 avg | **3** |
